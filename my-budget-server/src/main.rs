...
lazy_static! {
    ...
    static ref CLIENT: Mutex<Client> = Mutex::new(Client::connect(&DB_URL, NoTls).unwrap());
}
...
fn main() {
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:8080")).await.unwrap();
...
async fn handle_client(mut stream: TcpStream) {
    ...
    match stream.read(&mut buffer).await {
        Ok(size) => {
            ...
            if let Err(e) = stream.write_all(format!("{}{}", status_line, content).as_bytes()).await {
